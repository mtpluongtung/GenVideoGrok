...
export function buildVideoDescriptionPrompt({
  ...
  return [
    ...
    '4. ÂM THANH & THOẠI: Phân tích cử chỉ, ngữ cảnh và khẩu hình miệng của nhân vật để xác định hoặc tái hiện chính xác câu thoại, lời nói, lời dẫn phù hợp với tình huống và cảm xúc trong từng cảnh. Chú ý các phụ đề hoặc chữ xuất hiện trong hình để ghi lại câu thoại nguyên văn của nhân vật.',
...
export function buildStoryboardPrompt({
  ...
  return [
    ...
    outputLanguage === 'none'
      ...
      : '- RECREATE CHARACTER DIALOGUE & SPEECH: For parts where characters speak, interact, or express emotion, write their exact spoken lines in single quotes \'...\' (for example: Character says: \'...\') with lip-sync delivery instructions so Grok generates character voice and speech in the video instead of a silent clip.',

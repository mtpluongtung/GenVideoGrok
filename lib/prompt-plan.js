...
export function buildVideoDescriptionPrompt({
  ...
  return [
    ...
    userNote,
    langNote,
    'NHIỆM VỤ: Phân tích toàn diện và mô tả chi tiết, chân thực 100% nội dung của video này. Tuyệt đối không suy đoán hay thêm thắt những chi tiết không có trong video.',
    'Hãy trình bày bản mô tả nội dung video theo cấu trúc đầy đủ sau:',
    '1. CHỦ THỂ & NHÂN VẬT: Ai hoặc cái gì là trung tâm của video? (Ngoại hình, độ tuổi, trang phục, màu tóc, khuôn mặt, cử chỉ, biểu cảm nổi bật).',
    '2. BỐI CẢNH & KHÔNG GIAN: Video diễn ra ở đâu? (Không gian trong nhà hay ngoài trời, ánh sáng, gam màu chủ đạo, thời gian trong ngày, các vật thể xung quanh).',
    '3. DIỄN BIẾN HÀNH ĐỘNG THEO THỜI GIAN: Mô tả chi tiết trình tự hành động diễn ra từ đầu video, giữa video đến cuối video.',
    '4. ÂM THANH & THOẠI: KHÔNG có âm thanh gốc, chỉ có hình. Chỉ nêu những gì suy ra chắc chắn từ hình (khẩu hình đang nói, nhạc cụ đang chơi, chữ trên màn hình); tuyệt đối không bịa lời thoại.',
    '5. THÔNG ĐIỆP & TỔNG KẾT: Mục đích, phong cách và thông điệp cốt lõi của video là gì? (Ví dụ: giới thiệu sản phẩm chăm sóc da, chia sẻ phong cách sống, hướng dẫn nấu ăn, phong cảnh thiên nhiên...).',
...
export function buildStoryboardPrompt({
  ...
  return [
    ...
    'Analyze the uploaded contact-sheet frames sampled from the source video. The original audio is not available, so infer sound only where the frames make it unambiguous.',
 ...
 */
export const JSON_CODE_BLOCK_RULE = 'OUTPUT FORMAT: put the entire JSON object inside ONE fenced code block that starts with ```json and ends with ```, and write nothing outside that block. Outside a code block the chat interface strips the backslash from \\" and breaks the JSON. Inside string values prefer single quotes or curly quotes “ ” for quoted words (for example: a sign reading \'I am hungry\'); if you must use a straight double quote, escape it as \\".';

/**
 * Escapes a straight quote that sits inside a JSON string value but does not end it. A real closing
 * quote is followed (after whitespace) by , } ] or :, so anything else is treated as text. Raw line
 * breaks inside a string are escaped too. It is a heuristic — a quote directly followed by a comma
 * inside prose is still misread — which is why callers also retry.
 */

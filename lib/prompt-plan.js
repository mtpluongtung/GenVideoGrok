...
export function buildGeminiStoryPrompt({ duration, outputLanguage = 'auto' }) {
  ...
  return [
    ...
    `The source video duration measured locally is ${sourceDuration.toFixed(2)} seconds. Use this video as the visual foundation and core inspiration, but do NOT limit the story to a short summary of the clip. Expand it into a comprehensive, deep, full-length literary prose story.`,
    'MANDATORY MINIMUM LENGTH REQUIREMENT: The story MUST BE OVER 2,000 WORDS (bắt buộc độ dài tối thiểu trên 2.000 từ). Do not provide a brief summary, outline, or truncated scene.',
    'NARRATIVE AND LITERARY REQUIREMENTS:',
    '- Develop a multi-chapter or multi-section narrative arc with detailed world-building, vivid sensory details (visuals, sounds, atmosphere), and historical or environmental context.',
    '- Deepen characters with rich backstories, psychological depth, internal monologues, and emotional motivations.',
    '- Include extensive, realistic dialogues and interactions between characters that advance the conflict.',
    '- Build steady rising tension, a dramatic emotional or narrative climax, and a thoughtful, lingering conclusion.',
    ...
    '  "content": "the complete, full-length prose story exceeding 2,000 words with rich multi-paragraph development; use double newlines (\\\\n\\\\n) between paragraphs"',
    '}',
...
export function buildGeminiStoryRepairPrompt(error, { outputLanguage = 'auto' } = {}) {
  return [
    ...
    'MANDATORY LENGTH REQUIREMENT: The story MUST BE OVER 2,000 WORDS (bắt buộc độ dài tối thiểu trên 2.000 từ). Do not summarize, truncate, or write a short scene. Expand the narrative with rich descriptive paragraphs, detailed dialogues, character psychology, and vivid world-building.',
    storyLanguageInstruction(outputLanguage),
    'Return exactly one valid JSON object with two non-empty string fields: "title" and "content".',
    'The "content" field must contain the complete prose story exceeding 2,000 words, closely grounded in the video, with paragraphs separated by \\n\\n. Do not return Markdown, a storyboard, prompts, timestamps, or commentary.'
  ].join('\n');
}
...
export function buildGeminiMasterWriterPrompt({
  ...
  const contextInstruction = hasSource
    ? `Dựa trên video nguồn đã tải lên (thời lượng ~${Number(duration || 10).toFixed(1)} giây). Hãy quan sát kỹ từng chi tiết nhân vật, hành động, bối cảnh và cảm xúc trong video làm hạt nhân để mở rộng thành một tác phẩm văn học trường đoạn hoàn chỉnh, đồ sộ và sâu sắc.`
    : userPrompt?.trim()
      ? `Chủ đề sáng tác: "${userPrompt.trim()}". Hãy phát triển ý tưởng này thành một kiệt tác văn học trường đoạn đồ sộ và độc đáo.`
      : 'Hãy sáng tạo một tác phẩm văn học nghệ thuật trường đoạn độc bản, giàu tính triết lý, nhân văn và cuốn hút.';
...
export function buildGeminiMasterWriterRepairPrompt(error, { outputLanguage = 'auto' } = {}) {
  const storyLangInstruction = storyLanguageInstruction(outputLanguage);
  return [
    `Phản hồi trước của bạn chưa đạt yêu cầu: ${error.message}`,
    'Với vai trò Nhà văn kiệt xuất, hãy viết lại tác phẩm hoàn chỉnh, ĐẢM BẢO ĐỘ DÀI BẮT BUỘC TRÊN 2.000 TỪ (hơn 2.000 words).',
    'Hãy mở rộng sâu sắc bối cảnh không gian, miêu tả tâm lý và độc thoại nội tâm nhân vật, các màn đối thoại chi tiết và trường đoạn cao trào để dung lượng tác phẩm vượt trên 2.000 từ. Tuyệt đối không tóm tắt hay cắt ngắn.',
    storyLangInstruction,

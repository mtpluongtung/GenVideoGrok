...
export function buildGeminiVideoDescriptionPrompt({
  duration,
  userInstruction = '',
  outputLanguage = 'auto'
} = {}) {
  ...
  return [
    ...
    `Hãy xem kỹ toàn bộ video đã tải lên (thời lượng ~${sourceDuration.toFixed(1)} giây).`,
    userNote,
    langNote,
    ...
    '4. ÂM THANH & THOẠI: Video có lời nói, nhạc nền hay âm thanh hiện trường gì không? (Nếu có hãy ghi lại nội dung hoặc đặc trưng âm thanh).',
...
export function parseGeminiVideoDescription(text) {
  if (!text || typeof text !== 'string') {
    throw new Error('Gemini chưa trả về văn bản mô tả nội dung video.');
  }

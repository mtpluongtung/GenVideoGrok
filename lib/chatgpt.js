...
export function describeContactSheets(sheets = []) {
  ...
  return [
    'SOURCE FRAMES: the original video file cannot be uploaded to ChatGPT, so it was replaced by the attached contact-sheet images. Each sheet is a grid of frames sampled evenly across the video, read left-to-right then top-to-bottom in time order. The original audio track is NOT available directly: observe the characters, lips movement, facial expressions, and narrative context to reconstruct their authentic dialogue, spoken words, and emotional voice-over.',

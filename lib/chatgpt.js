...
export function describeContactSheets(sheets = []) {
  ...
  return [
    'SOURCE FRAMES: the original video file cannot be uploaded to ChatGPT, so it was replaced by the attached contact-sheet images. Each sheet is a grid of frames sampled evenly across the video, read left-to-right then top-to-bottom in time order. The user requires the recreated video to have full character voice and spoken dialogue. Observe character mouth movements, expressions, and context to recreate and write exact spoken dialogue lines in quotes and lip-sync delivery for every speaking scene.',
...
async function isSendButtonEnabled(send) {
  if (!send) return false;
  const visible = await send.isVisible().catch(() => false);
  if (!visible) return false;
  const disabledAttr = await send.getAttribute('disabled').catch(() => null);
  if (disabledAttr !== null) return false;
  const ariaDisabled = await send.getAttribute('aria-disabled').catch(() => null);
  if (ariaDisabled === 'true') return false;
  const visuallyDisabled = await send.getAttribute('data-visually-disabled').catch(() => null);
  if (visuallyDisabled !== null) return false;
...
async function fillAndSend(page, prompt, log, attempt, isCancelled) {
  ...
  while (consecutiveReady < 2) {
    throwIfCancelled(isCancelled, 'Đã hủy trong lúc chờ ChatGPT sẵn sàng nút Gửi.');
    if (Date.now() - waitStarted > 180000) {
      throw new Error('ChatGPT chưa xử lý xong tệp đính kèm sau 3 phút; nút Gửi vẫn bị khóa.');
    }
    const ready = await isSendButtonEnabled(send);
    if (ready) {
      consecutiveReady += 1;
      if (consecutiveReady < 2) await page.waitForTimeout(1000);
    } else {
      consecutiveReady = 0;
  ...
  try {
    ...
    await log('chatgpt.submit.click_retry', { error: sanitizeError(clickError) });
    try {
      await box.press('Enter');
    } catch {
      await send.evaluate((btn) => btn.click()).catch(() => {});
    }
  }
...
export async function analyzeVideoWithChatGPT(job, metadata, onStatus = () => {}, { isCancelled, videoDescription = '', story = null } = {}) {
  ...
  const generatePlan = async (framesNote = '') => {
  ...
  // Nếu đã có bản phân tích video chi tiết từ Bước 1 (videoDescription), không cần upload lại contact sheet
  // nhằm tránh chạm ngưỡng giới hạn tệp đính kèm trên tài khoản ChatGPT (attachments_limit) và tránh lỗi khóa nút gửi.
  if (videoDescription?.trim()) {
    await log('chatgpt.storyboard.use_video_description', {
  ...
  return withSourceFrames({ job, duration: metadata?.duration, page, log, onStatus, isCancelled }, generatePlan);
}
...
async function latestGeneratedImage(page, previousImageCount) {
  const selectors = [
    `${ASSISTANT_SELECTOR} img`,
    `${TURN_SELECTOR} img`,
    'main img[alt*="image" i]',
    'main img[src*="oaiusercontent"]',
    'main img[src*="blob:"]'
  ].join(', ');
  const images = page.locator(selectors);
...
export async function generateStoryThumbnailWithChatGPT(job, story, onStatus = () => {}, { isCancelled = null } = {}) {
  ...
  onStatus('Đang mở ChatGPT để tạo ảnh minh họa…');
  const page = await chatgptPage();
  await openFreshChat(page, log, 'thumbnail', isCancelled);
  onStatus('ChatGPT đang vẽ ảnh minh họa 16:9 cho bài viết…');
  ...
  try {
    ...
    while (Date.now() - startWait < imageTimeout) {
      ...
      pollCount += 1;
      const elapsedSeconds = Math.round((Date.now() - startWait) / 1000);
      if (pollCount % 5 === 0) {
        onStatus(`ChatGPT đang vẽ ảnh minh họa (${elapsedSeconds}s)…`);
      }
      ...
      if (image && !stopVisible) {
        onStatus('Đang tải ảnh từ ChatGPT về máy…');
        await page.waitForTimeout(2000);

        // 1. Tải trực tiếp luồng nhị phân nguyên gốc của ảnh qua browser fetch
        const imgSrc = await image.getAttribute('src').catch(() => null);
        if (imgSrc) {
          try {
            ...
            if (bufferArray && bufferArray.length > 3000) {
              await fs.writeFile(thumbnailPath, Buffer.from(bufferArray));
              imageCaptured = true;
              await log('chatgpt.thumbnail.downloaded', { bytes: bufferArray.length, url: imgSrc.slice(0, 100) });
              break;
            }
          } catch (fetchErr) {
            await log('chatgpt.thumbnail.fetch_warning', { error: sanitizeError(fetchErr) });
          }
        }

        // 2. Dự phòng chụp ảnh phần tử nếu tải file trực tiếp gặp trở ngại
        await image.screenshot({ path: thumbnailPath });
        const stat = await fs.stat(thumbnailPath).catch(() => null);
        if (stat && stat.size > 3000) {
          imageCaptured = true;
      ...
      // Kiểm tra xem ChatGPT có hiển thị thông báo lỗi sau khi đã đợi ít nhất 45 giây
      if (!image && !stopVisible && Date.now() - startWait > 45000) {
        const hasError = await page.locator('.text-red-500, [class*="error"], [data-testid*="error"]')
          .filter({ visible: true }).first().isVisible().catch(() => false);
        if (hasError) {
          await log('chatgpt.thumbnail.error_detected');
          break;
        }
      }

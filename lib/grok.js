...
export function isSameRoute(currentUrl, targetUrl) {
  try {
...
export async function configureGrokSettings(aspectRatio = null) {
  ...
  if (!isSameRoute(page.url(), grokUrl)) await page.goto(grokUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
  let verified;
...
async function selectRadio(page, radioOrGetRadio, label, timeoutMs) {
  const startedAt = Date.now();
  while (Date.now() - startedAt < timeoutMs) {
    const radio = typeof radioOrGetRadio === 'function' ? await radioOrGetRadio() : radioOrGetRadio;
...
export async function openGrokImaginePage(ctx, initialPage, log, { attempts = 3, readyTimeoutMs = 30000, navigationTimeoutMs = 60000, url = grokUrl, reuseTimeoutMs = 3000, isCancelled = null } = {}) {
  ...
  if (initialPage?.url() && isSameRoute(initialPage.url(), url)) {
...
export async function ensureVideoSettings(page, log, phase, { timeoutMs = 30000, aspectRatio = null, clipSeconds = 10 } = {}) {
  if (page.url() && page.url().includes('grok.com') && !isSameRoute(page.url(), grokUrl)) {
    await log('grok.settings.navigating_to_imagine', { from: page.url(), to: grokUrl });
    await page.goto(grokUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await waitForGrokShell(page, timeoutMs);
  }
  ...
  const getVideoMode = async () => {
    const primary = page.getByRole('radio', { name: /^video$/i }).filter({ visible: true }).first();
    if (await primary.isVisible().catch(() => false)) return primary;
    const byText = page.getByRole('radio', { name: /video/i }).filter({ visible: true }).first();
    if (await byText.isVisible().catch(() => false)) return byText;
    const buttonRadio = page.locator('button[role="radio"]').filter({ hasText: /^video$/i, visible: true }).first();
    if (await buttonRadio.isVisible().catch(() => false)) return buttonRadio;
    const generalButton = page.getByRole('button', { name: /^video$/i }).filter({ visible: true }).first();
    if (await generalButton.isVisible().catch(() => false)) return generalButton;
    return primary;
  };
  await selectRadio(page, getVideoMode, 'Video', timeoutMs);

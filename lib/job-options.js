...
export function videoLanguageInstruction(value = 'auto', { hasSource = false } = {}) {
  ...
  if (language === 'auto') {
    return hasSource
      ? 'Preserve the spoken language of the source video and recreate character dialogue and narration faithfully. Write character spoken lines verbatim in quotation marks with natural lip-sync delivery.'
      : 'Use the language naturally implied by the user concept. Include expressive, natural dialogue and narration written verbatim in quotation marks.';
  }
  const name = GENERATION_LANGUAGE_NAMES[language];
  return `Spoken dialogue, character voice-over, and narration must be in ${name}. ` +
    `${hasSource ? `Translate and recreate the source speech faithfully into ${name} with exact spoken lines in quotes and lip-sync delivery. ` : 'Include clear, natural spoken lines written verbatim in quotation marks. '}` +

...
export function videoLanguageInstruction(value = 'auto', { hasSource = false } = {}) {
  ...
  if (language === 'auto') {
    return hasSource
      ? 'Preserve the spoken language of the source video and recreate character dialogue and narration faithfully. Characters must audibly speak their lines with synchronized lip movements. Include rich Foley and ambient sounds.'
      : 'Use the language naturally implied by the user concept. Include audible, natural spoken dialogue with lip-sync and rich sound effects.';
  }
  const name = GENERATION_LANGUAGE_NAMES[language];
  return `Spoken dialogue, character voice-over, and narration must be in ${name}. ` +
    `${hasSource ? `Translate and recreate the source speech faithfully into ${name} with exact spoken lines in quotes and natural lip-sync delivery. ` : 'Include clear, natural spoken lines written verbatim in quotation marks with lip-sync delivery. '}` +
    'Include realistic environmental sound effects and voice audio.';
}
...
export const DEFAULT_CLIP_PREFERENCE = Object.freeze([10, 15, 5]);
...
export function parseTargetDuration({ type, durationMode, durationSeconds, maxParts = 30, clipSeconds = 10 }) {
  ...
  const rawMode = String(durationMode || defaultMode).trim().toLowerCase();

  let mode = rawMode;
  let explicitSeconds = durationSeconds;
  if (/^\d+$/.test(rawMode)) {
    mode = 'custom';
    explicitSeconds = Number(rawMode);
  }
  ...
  const fallback = type === 'topic' && (explicitSeconds == null || explicitSeconds === '') ? step : explicitSeconds;

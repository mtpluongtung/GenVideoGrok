...
export async function describeVideoWithGemini(job, metadata, onStatus = () => {}, { isCancelled } = {}) {
  ...
  onStatus('Đang mở Gemini để phân tích nội dung video…');
  ...
  onStatus('Đang tải video nguồn lên Gemini để phân tích nội dung…');
  ...
  const instruction = buildGeminiVideoDescriptionPrompt({
    duration: metadata.duration,
    userInstruction: job.prompt || '',
    outputLanguage: job.language || 'auto'
  });

  onStatus('Gemini đang xem và phân tích toàn diện nội dung video…');
  let raw = await submitAndRead(page, instruction, log, 1, onStatus, isCancelled);
  await saveRawResponse(job.id, raw, 1, log, 'describe');

  try {
    const videoDescription = parseGeminiVideoDescription(raw);
    await log('gemini.video_description.validated', {
      repaired: false,
    ...
    await log('gemini.video_description.invalid', { attempt: 1, error: sanitizeError(error) });
    onStatus('Gemini đang hoàn thiện bản mô tả nội dung video…');
    const repairPrompt = [
      `Phản hồi trước chưa đạt yêu cầu: ${error.message}`,
      'Hãy xem kỹ video đã tải lên và mô tả đầy đủ, chi tiết nội dung video theo 5 phần: Chủ thể & Nhân vật, Bối cảnh & Không gian, Diễn biến hành động theo thời gian, Âm thanh & Thoại, Thông điệp & Tổng kết.'
    ].join('\n');
    raw = await submitAndRead(page, repairPrompt, log, 2, onStatus, isCancelled);
...
export async function analyzeVideoWithGemini(job, metadata, onStatus = () => {}, { isCancelled, videoDescription = '', story = null } = {}) {
  ...
  await assertSignedIn(page);
  await log('gemini.auth.ok', { url: page.url() });
  onStatus('Đang tải video nguồn lên Gemini để viết prompt cho Grok tạo video tương tự…');
  throwIfCancelled(isCancelled, 'Đã hủy trước khi tải video lên Gemini.');
  await uploadVideo(page, job.sourcePath, log);
  throwIfCancelled(isCancelled, 'Đã hủy sau khi tải video lên Gemini.');
  ...
  onStatus(`Gemini đang viết ${expectedParts} prompt cho Grok tạo video tương tự…`);
  let raw = await submitAndRead(page, instruction, log, 1, onStatus, isCancelled);
...
export async function generateStoryFromVideoWithGemini(job, metadata, onStatus = () => {}, { isCancelled, videoPlan = null, userPrompt = '', videoDescription = '' } = {}) {
  ...
  // Khi đã có videoDescription từ Lần 1, gửi trực tiếp nội dung video đã phân tích mà không cần upload lại file
  ...
  const planParts = videoPlan?.parts || [];
  const promptText = userPrompt || job.prompt || '';
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    throwIfCancelled(isCancelled, `Đã hủy tại lần thử ${attempt} sáng tác câu chuyện.`);
    let currentInstruction = '';
    if (attempt === 1) {
      currentInstruction = instruction;
      onStatus('Gemini đang sáng tác câu chuyện dài trên 2.000 từ bám sát video nguồn…');
    } else {
      currentInstruction = buildGeminiStoryRepairPrompt(lastError, {
        outputLanguage: job.language || 'auto',
        attempt,
        videoDescription,
        videoSummary,
      ...
      onStatus(`Gemini đang hoàn thiện câu chuyện trên 2.000 từ (lần thử ${attempt}/${MAX_ATTEMPTS})…`);
    }

    raw = await submitAndRead(page, currentInstruction, log, attempt, onStatus, isCancelled);
    await saveRawResponse(job.id, raw, attempt, log, 'story');

    try {
      const minRequiredWords = attempt === 1 ? MIN_STORY_WORDS : MIN_STORY_WORDS_RETRY;
      const story = parseGeminiStory(raw, { minWords: minRequiredWords });
      ...
      return { story, rawResponse: raw, repaired: attempt > 1 };
    } catch (error) {
      if (isCancellation(error)) throw error;
      lastError = error;
      await log('gemini.story.invalid', { attempt, error: sanitizeError(error) });
      if (attempt === MAX_ATTEMPTS) {
        throw new Error(`Gemini không tạo được câu chuyện hợp lệ sau ${MAX_ATTEMPTS} lần thử: ${error.message}`);
      }
    }
  }
}
...
export async function generateMasterStoryWithGemini(job, onStatus = () => {}, { isCancelled, videoPlan = null } = {}) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    throwIfCancelled(isCancelled, `Đã hủy tại lần thử ${attempt} sáng tác tác phẩm.`);
    let currentInstruction = '';
    if (attempt === 1) {
      currentInstruction = instruction;
      onStatus('Nhà văn kiệt xuất (Gemini) đang sáng tác tác phẩm trường đoạn trên 2.000 từ…');
    } else {
      currentInstruction = buildGeminiMasterWriterRepairPrompt(lastError, {
        outputLanguage: job.language || 'auto',
        attempt,
        videoSummary,
        globalContinuity
      });
      onStatus(`Gemini đang trau chuốt và mở rộng tác phẩm trên 2.000 từ (lần thử ${attempt}/${MAX_ATTEMPTS})…`);
    }

    raw = await submitAndRead(page, currentInstruction, log, attempt, onStatus, isCancelled);
    await saveRawResponse(job.id, raw, attempt, log, 'master_story');

    try {
      const minRequiredWords = attempt === 1 ? MIN_STORY_WORDS : MIN_STORY_WORDS_RETRY;
      const story = parseGeminiMasterStory(raw, { minWords: minRequiredWords });
      ...
      if (isCancellation(error)) throw error;
      lastError = error;
      await log('gemini.master_story.invalid', { attempt, error: sanitizeError(error) });
      if (attempt === MAX_ATTEMPTS) {
        throw new Error(`Gemini không tạo được tác phẩm văn học hợp lệ sau ${MAX_ATTEMPTS} lần thử: ${error.message}`);
      }
    }
  }
}

...
export async function generateStoryFromVideoWithGemini(job, metadata, onStatus = () => {}, { isCancelled } = {}) {
  ...
  try {
    ...
    const story = parseGeminiStory(raw);
    await log('gemini.story.validated', {
      repaired: true, titleLength: story.title.length, contentLength: story.content.length
    });
...
export async function generateTopicStoryWithGemini(job, onStatus = () => {}, { isCancelled } = {}) {
  ...
  await saveRawResponse(job.id, raw, 1, log, 'story');
  try {
    const plan = parseGeminiTopicStoryPlan(raw, { targetDuration, clipSeconds });
    ...
    return {
      story: { title: plan.title, content: plan.content },
      plan,
      rawResponse: raw,
      repaired: false
    };
    ...
    onStatus('Gemini đang chuẩn hóa lại câu chuyện…');
...
export async function generateMasterStoryWithGemini(job, onStatus = () => {}, { isCancelled } = {}) {
  ...
  onStatus('Nhà văn kiệt xuất (Gemini) đang sáng tác tác phẩm văn học nghệ thuật…');
  let raw = await submitAndRead(page, instruction, log, 1, onStatus, isCancelled);
  await saveRawResponse(job.id, raw, 1, log, 'master_story');

  try {
    const story = parseGeminiMasterStory(raw);
    ...
    onStatus('Gemini đang trau chuốt lại cấu trúc câu chuyện…');
